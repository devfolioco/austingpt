...
- Square aspect ratio, at least 512x512px. GIF or PNG both work (update the file extension in `persona.config.ts` if you switch formats).
...
- These are composited onto a canvas at render time. Keep them as portrait cutouts (head + shoulders) with transparent backgrounds, roughly 400x500px.

**OG image** (`og-image-1.1.png`)
- 1200x630px (standard Open Graph ratio). Include your app name and a short tagline — this is what shows up in link previews on Twitter/Farcaster/iMessage.
